...
export default function ChatScreen({
  ...
  //MISSING: this is only run once at the first render
  useEffect(() => {
  ...
  //MIssing: runs upon every render
  useEffect(() => {
    return () => {
  ...
  const handleBlock = async () => {
    ...
    if (data) {
      ...
      //missing: we need to update the chat history here, temp setting sethidebottom bar to see if the useEffect runs one time is causing the issue
      // setHideBottomBar(false);
      navigation.goBack();
    }
  };
